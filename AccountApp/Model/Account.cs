...
namespace AccountApp.Model
{
    internal class Account
    {
        ...
        public void Deposit(decimal amount)
        {
            try
            {
                if (amount < 0) throw new NegativeAmountException($"Amount {amount} must not be negative");
                Balance += amount;
            } 
        ...
        public void Withdraw(decimal amount, string? ssn)
        {
            try
            {
                if (string.IsNullOrEmpty(ssn)) throw new InvalidSsnException($"{ssn} must not be null or empty");
                if(!string.Equals(ssn, Ssn)) throw new InvalidSsnException();
            ...
            {
                Console.Error.WriteLine(e.Message); throw;
            }
            //catch (NegativeAmountException e)
            //{
            //    Console.Error.WriteLine(e.Message);
